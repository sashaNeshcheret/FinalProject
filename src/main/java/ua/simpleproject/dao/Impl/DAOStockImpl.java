...
public class DAOStockImpl implements DAOStock {
    public static final String SQL_UPDATE = "UPDATE stock SET number = ? WHERE product_id = ?";
    ...
    /** Read from table "Stock" objects which start from index start to end
     *
     * @param start
     * @param end
     * @return
    ...
    /**Select information from table "Stock" according to current product
     *
     * @param product
     * @return
     * @throws DAOException this is own exception that combines exceptions which
     * happened during work with database
     * @throws LogicException Inform that quantity of products in stock less than needed
     */
    public Stock read(Product product) throws DAOException, LogicException {
        Stock stock = new Stock();
        ResultSet resultSet = null;
        try(ConnectionWrapper connection = TransactionManager.getConnection()) {
            String sqlSelect = "SELECT * FROM stock WHERE product_id = (" +
                    "SELECT id FROM products WHERE code_product = ?)";
            ...
            throw new DAOException("From method readStock hasn't got product"+ product, e);
        }
        return stock;
    }

    /**Refresh table "Stock" by object stock
     *
     *
     * @param stock
     * @throws DAOException this is own exception that combines exceptions which
    ...
    /**Create object "stock" in base of product and quantity
     *
     * @param product
     * @param number
     * @throws DAOException this is own exception that combines exceptions which
